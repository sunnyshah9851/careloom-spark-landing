...
const AddRelationshipModal = ({ open, onOpenChange, onRelationshipAdded }: AddRelationshipModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      sonnerToast.error('You must be logged in to add relationships');
      return;
    }

    if (!formData.name || !formData.relationship_type || !formData.birthday) {
      sonnerToast.error('Please fill in the name, relationship type, and birthday fields');
      return;
    }
    ...
    if (isPartnerRelationship && !formData.email) {
      sonnerToast.error('Email is required for partner and spouse relationships');
      return;
    }
    ...
    (isPartnerRelationship)
      ...
      sonnerToast.success(`${formData.name} has been added to your relationships!`);
      ...
      sonnerToast.error('Failed to add relationship. Please try again.');
    } finally {
      setLoading(false);
    }
  };
